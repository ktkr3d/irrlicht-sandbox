...
const char* LIGHT_MODULATE_P[ESE_COUNT] = {"uniform sampler2D ColorMapSampler;\n"
        ...
        ""
        "	gl_FragColor =  lightCol+finalCol;\n"
        "}"
        ,
...
const char* SHADOW_PASS_1P[ESE_COUNT] = {"void main() "
                                        ...
                                        "depth+=(gl_TexCoord[1].y)/100.0;"
                                        "   gl_FragColor =vec4(fract(depth*vec4(1.0, 256.0, 65536.0, 1.0)).xyz,1.0);\n"
                                        "}"
                                        ,
...
const char* SHADOW_PASS_1PT[ESE_COUNT] = {"uniform sampler2D ColorMapSampler;\n"
        ...
        ""
        "    gl_FragColor = vec4(fract(depth*vec4(1.0, 256.0, 65536.0, 1)).xyz,alpha);\n"
        "}"
        ,
        "sampler2D ColorMapSampler : register(s0);\n"
        ""
...
const char* SHADOW_PASS_2P[ESE_COUNT] = {"uniform sampler2D ShadowMapSampler;\n"
                                        ...
                                        "}\n"
                                        "##else\n"
                                        "float testShadow(vec2 smTexCoord, vec2 offset, float realDistance,float lol)"
                                        "{"
                                        "	vec4 texDepth = texture2D(ShadowMapSampler, vec2( smTexCoord + offset));\n"
                                        "	float extractedDistance = dot(texDepth,vec4(1.0, 0.00390625, 0.0000152587890625, 0.0));\n"
                                        "	float factor=20.0;"
                                        "	return clamp(exp(30.0*(extractedDistance-realDistance)),0.0,1.0);// ? (1.0  / float(SAMPLE_AMOUNT)) : 0.0;\n"
                                        "}\n"
                                        "##endif\n"
                                        "\n"
                                        "vec2 offsetArray[16];\n"
                                        "vec4 DecodeRGBD(vec4 rgbd)"
                                        "{"
                                        "    return vec4(rgbd.rgb * ((65025.0 / 255.0) / rgbd.a),0.0);"
                                        "}"

                                        "vec4 EncodeRGBD(vec3 rgb)"
                                        "{"
                                        "   float maxRGB = max(rgb.x,max(rgb.g,rgb.b));"
                                        "  float D      = max(65025.0 / maxRGB, 1.0);"
                                        " D            = clamp(floor(D) / 255.0,0.0,1.0);"
                                        "return vec4(rgb.rgb * (D * (255.0 / 65025.0)), D);"
                                        "}"
                                        "void main() \n"
                                        "{"
                                        "	vec4 SMPos = gl_TexCoord[0];\n"
                                        "	vec4 MVar = gl_TexCoord[1];\n"
                                        "	vec3 nr = gl_TexCoord[2].xyz;\n"
                                        "	vec3 ldir= gl_TexCoord[3].xyz;\n"
                                        "float len=1.0-clamp(length(ldir)/MVar.z,0.0,1.0);\n"
                                        "ldir=normalize(ldir);"
                                        "nr=normalize(nr);"
                                        ...
                                        "		float realDist = MVar.x / MVar.z-(0.005/dot(ldir,nr));\n"
                                        "	"
                                        "		for(int i = 0;i < SAMPLE_AMOUNT; i++)"
                                        "			lightFactor *= testShadow(SMPos.xy, offsetArray[i] * MVar.w, realDist,gl_TexCoord[1].y);\n"

                                        "		// Multiply with diffuse.\n"
                                        "##ifdef ROUND_SPOTLIGHTS\n"
                                        "		finalCol = DecodeRGBD(LightColour) * lightFactor*len*dot(ldir,nr)* clamp(5.0 - 10.0 * lengthToCenter, 0.0, 1.0);\n"
                                        "##else\n"
                                        "		finalCol =DecodeRGBD(LightColour) * lightFactor*clamp(len,0.0,1.0)*clamp(dot(ldir,nr),0.0,1.0);\n"

                                        "##endif\n"
                                        "finalCol.a=finalCol.r-1.0;"
                                        "	}"

                                        "	gl_FragColor =EncodeRGBD(finalCol.xyz);\n"
                                        "}"
                                        ,
                                        ...
                                        "##else\n"
                                        "	float2 clampedSMPos = clamp(SMPos.xy,0.0,1.0);\n"
                                        ...
                                        "	"
                                        "		for(int i = 0;i < SAMPLE_AMOUNT; ++i)"
                                        "			lightFactor -= calcShadow(SMPos.xy, offsetArray[i] * MVar[2], realDistance,gl_TexCoord[1]);\n"
                                        ""
                                        "		// Multiply with diffuse.\n"
                                        "##ifdef ROUND_SPOTLIGHTS\n"
...
const char* SHADOW_PASS_2VS[ESE_COUNT] = {"struct VS_OUTPUT "
        "{"
        "	vec4 Position;\n"
        "	vec4 ShadowMapSamplingPos;\n"
        "	vec4 MVar;\n"
        ...
        "	"
        "	OUT.MVar.x = OUT.ShadowMapSamplingPos.z;\n"
        "	OUT.MVar.y = dot(normalize(gl_Normal.xyz), normalize(lightDir));\n"
        "	OUT.MVar.z = MaxD;\n"
        "	OUT.MVar.w = 1.0 / MAPRES;\n"
        ...
        "{"
        "	VS_OUTPUT vOut = vertexMain(gl_Vertex.xyz);\n"
        "   vec2 scrn=gl_MultiTexCoord1.xy*2.0-1.0; "

        "	gl_Position = vec4(scrn, 0.0, 1.0);\n"
        "	gl_TexCoord[0] = vOut.ShadowMapSamplingPos;\n"
        "	gl_TexCoord[1] = vOut.MVar;\n"
        "	gl_TexCoord[3].xyz = vOut.lightdir;\n"
        "gl_TexCoord[2].xyz=gl_Normal.xyz; \n"
        "}"
        ,
        "float4x4 mWorldViewProj;\n"
        "float4x4 mWorldViewProj2;\n"
...
const char* VSM_BLUR_P[ESE_COUNT] = {"uniform sampler2D ColorMapSampler;\n"
                                     ...
                                     "	offsetArray[0] = vec3(0.0, 0.0,1.0);\n"
                                     "	offsetArray[1] = vec3(0.0, -1.0 / gl_TexCoord[3].y,1.0625);\n"
                                     "	offsetArray[2] = vec3(0.0, 1.0 / gl_TexCoord[3].y,1.0625);\n"
                                     "	offsetArray[3] = vec3(0.0, -2.0 / gl_TexCoord[3].y,1.125);\n"
                                     "	offsetArray[4] = vec3(0.0, 2.0 / gl_TexCoord[3].y,1.125);\n"
                                     "##else\n"
                                     "	offsetArray[0] = vec3(0.0, 0.0,1.0);\n"
                                     "	offsetArray[1] = vec3(-1.0 / gl_TexCoord[3].x, 0.0,1.0625);\n"
                                     "	offsetArray[2] = vec3(1.0 / gl_TexCoord[3].x, 0.0,1.0625);\n"
                                     "	offsetArray[3] = vec3(-2.0 / gl_TexCoord[3].x, 0.0,1.125);\n"
                                     "	offsetArray[4] = vec3(2.0 / gl_TexCoord[3].x, 0.0,1.125);\n"
                                     "##endif\n"
                                     "\n"
                                     ...
                                     "##endif\n"
                                     "vec4 tint;"
                                     "vec4 cent= texture2D(ScreenMapSampler,pos);"
                                     "float flag=cent.a;"

                                     "	for(int i = 0;i < 5;++i)\n"
                                     "{"
                                     "		vec4 tex= texture2D(ColorMapSampler,pos + offsetArray[i].xy);\n"

                                     "BlurCol+=tex;"

                                     //"if (tint>cint)cent=tex;"
                                     "flag= min(tex.a,flag);"
                                     "if(cent.a<tex.a)cent=tex;"
                                     "}"
                                     "\n"
                                     "	gl_FragColor = BlurCol/5.0;\n"
                                     "if(flag==0.0)	gl_FragColor =cent;\n"
                                     "}\n"
                                     ,
                                     "sampler2D ColorMapSampler : register(s0);\n"
                                     "\n"
                                     ...
                                     "##ifdef VERTICAL_VSM_BLUR\n"
                                     "	offsetArray[0] = float2(0.0, 0.0);\n"
                                     "	offsetArray[1] = float2(0.0, 1.5 / ScreenSize.y);\n"
                                     "	offsetArray[2] = float2(0.0, -1.5 / ScreenSize.y);\n"
                                     "	offsetArray[3] = float2(0.0, 2.5 / ScreenSize.y);\n"
                                     "	offsetArray[4] = float2(0.0, -2.5 / ScreenSize.y);\n"
                                     "##else\n"
                                     "	offsetArray[0] = float2(0.0, 0.0);\n"
                                     "	offsetArray[1] = float2(1.5 / ScreenSize.x, 0.0);\n"
                                     "	offsetArray[2] = float2(-1.5 / ScreenSize.x, 0.0);\n"
                                     "	offsetArray[3] = float2(2.5 / ScreenSize.x, 0.0);\n"
                                     "	offsetArray[4] = float2(-2.5 / ScreenSize.x, 0.0);\n"
                                     "##endif\n"
                                     "\n"
                                     ...
                                     "##endif\n"
                                     "	for(int i = 0;i < 2;++i)\n"
                                     "		finalVal += tex2D(ColorMapSampler,Texcoords.xy + offsetArray[i]);\n"
                                     "\n"
                                     "	return finalVal / 5.0;\n"
                                     "}\n"
                                    };
